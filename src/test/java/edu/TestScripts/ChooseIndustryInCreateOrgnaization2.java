...
public class ChooseIndustryInCreateOrgnaization2 extends BaseClass {
	...
	public void ChooseIndustryInCreateOrg() 
	{
		...
		Map<String, String> map = excelUtil.readDataFromExcel("Create Organization With Industry And Type", "OrganizationsTestData");
		...
		if(orgPage.verifyOrgNameInOrganizationPage().contains(orgName))
			excelUtil.setDataToExcel(IConstantPath.EXCEL_FILE_PATH, "Create Organization With Industry And Type", "pass", "OrganizationsTestData");
		else
			excelUtil.setDataToExcel(IConstantPath.EXCEL_FILE_PATH, "Create Organization With Industry And Type", "fail", "OrganizationsTestData");
		soft.assertAll();
